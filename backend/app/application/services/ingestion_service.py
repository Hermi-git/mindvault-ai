...
class IngestionService:
    def __init__(
        self, storage, repository, parser, embedder, vector_store, uow_factory
    ):
    ...
    async def process_document(self, document_id: str | UUID) -> None:
        ...
        try:
            ...
            logger.debug(
                "Extracted %d characters from document %s", len(text), document_id
            )
            ...
            if not embeddings or len(embeddings) != len(chunks):
                raise ValueError(
                    f"Failed to generate embeddings for all chunks in document {document_id}"
                )
            ...
            await self._vector_store.upsert(
                vectors=pinecone_payload, namespace=str(document.org_id)
            )
            logger.info(
                "Upserted %d vectors for document %s",
                len(pinecone_payload),
                document_id,
            )
